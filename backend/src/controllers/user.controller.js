...
const loginuser = asyncHandler(async (req, res, next) => {
  ...
  const userchecker = await User.findOne({ email: email });
  if (!userchecker) throw new errorhandler(400, "User does not exist");
  ...
  const { accessToken, refreshToken } =
    await generateAccessTokenandRefreshToken(userchecker._id);
  const loggedinuser = await User.findById(userchecker._id).select(
    "-password -refreshtoken",
  );
  const options = {
    httpOnly: true,
...
const updateuseravatar = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new responseHandler(200, "Profile updated successfully", updateduser),
    );
});
...
const returnuserProfile = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(
      new responseHandler(200, "User profile fetched successfully", req.user),
    );
});
...
const updateuseremail = asyncHandler(async (req, res) => {
  ...
  if (emailchecker) throw new errorhandler(400, "Email already in use");
  const updateduser = await User.findByIdAndUpdate(
    req.user._id,
    { $set: { email: newemail } },
    { new: true },
  ...
  return res
    .status(200)
    .json(new responseHandler(200, "Email updated successfully", updateduser));
});
export {
  registerUser,
  loginuser,
